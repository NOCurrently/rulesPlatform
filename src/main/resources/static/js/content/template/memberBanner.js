...
$(function () {
    ...
    dictValueLoadSelectSort("member_jumpType", "jump_type", path,jumpTypeDefVal);

    var targetCrowdId = $("#targetCrowdHidden").val();
    initTargetCrowd(targetCrowdId, "member_result");

    //提交数据
    $("#member_saveBtn").bind("click", function () {


        // 1.校验
        var isValiResult = validateMember();
        if(!isValiResult)
            return;

        // 提交前跳转地址再次确认
        var jumpUrl = $("#member_jumpUrl").val();
        var msg = "请确认跳转地址是否正确：\n"+jumpUrl;
        if(!confirm(msg)){
            return;
        }
        ...
        // 3.表单提交
        var infoId = $("#member_EditForm").find("input[name=infoId]").val();
        var menuId = $("#member_EditForm").find("input[name=menuId]").val();
        var title = $("#member_title").val();
        var subTitle = $("#member_subTitle").val();
        ...
        var priority = $("#member_priority").val();
        var jumpType = $("#member_jumpType").find('option:selected').val();
        //var jumpUrl = $("#member_jumpUrl").val();
        var dateParam = $("#timeParamJson").text();
        var customParam = $("#customParamJson").text();
        var remarks = $("#member_remarks").val();
        ...
        $.ajax({
            ...
            success: function (data) {
                if(200 == data.code){
                    ...
                    // 显示之前的错误信息
                    $('#member_result').html(data.errorMessage);
                    $('#member_result').show();
                }
            }
        });
    ...
    function validateMember() {
        ...
        var title = $("#member_title").val();
        if(isEmpty(title)){
            $('#member_result').html("名称不能为空！");
            $('#member_result').show();
            return false;
        }
        if(title.length > 100){
            $('#member_result').html("名称长度不能超过100！").show();
            return false;
        }
        var subTitle = $("#member_subTitle").val();
        if(subTitle.length > 150){
            $('#member_result').html("副标题长度不能超过150！").show();
            return false;
        }
        ...
        var jumpType = $("#member_jumpType").find('option:selected').val();
        if(isEmpty(jumpType)){
            $('#member_result').html("跳转类型不能为空！");
            $('#member_result').show();
            return false;
        }
        if(jumpType.length > 50){
            $('#member_result').html("跳转类型长度不能超过50！").show();
            return false;
        }
        // 5.时间设置
        ...
        if(!isEmpty(dateParam)){
            ...
            if(isEmpty(dateParamObj.beginTime)) {
                $('#member_result').html("时间设置开始日期不能为空！");
                $('#member_result').show();
                return false;
            }
        }
        ...
        if(remarks.length > 200){
            $('#member_result').html("备注长度不能超过200！").show();
            return false;
        }
        ...
        if(!isEmpty(priority)) {
            if (!(/^\+?[1-9][0-9]*$/).test(priority)) {
                $('#member_result').html("排序必须为正整数！");
                $('#member_result').show();
                return false;
            }
        }else{
            $("#member_priority").val(1);
        }
...
function initTargetCrowd (targetCrowdId) {
    ...
    if(!flag && (targetCrowdId !=0 || targetCrowdId !="")){
        $('#member_result').html("请输入正确的目标人群id").show();
        return;
    }else{
        $('#member_result').html("").hide();
    }
...
function getCrowdDetail(base,targetCrowdId) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if(200 == data.code){
                if(crowId > 0){
                    $("#inputTargetCrowdId").val("id=" + targetCrowdId);
                    var crowdHTML = "";
