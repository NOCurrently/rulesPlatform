...
$(function () {
    ...
    dictValueLoadSelectSort("pop-ad_dataCoding", dataCode, path,dataCodeDefVal,"请选择");

    var jumpTypeDefVal = $("#jumpTypeDefVal").val();
    dictValueLoadSelectSort("pop-ad_jumpType", "jump_type", path,jumpTypeDefVal);
    ...
    initTargetCrowd(targetCrowdId, "pop-ad_result");



    //提交数据
    $("#pop-ad_saveBtn").bind("click", function () {


        // 1.校验
        var isValiResult = validatePopAd();
        if(!isValiResult)
            return;

        // 提交前跳转地址再次确认
        var jumpUrl = $("#pop-ad_jumpUrl").val();
        var msg = "请确认跳转地址是否正确：\n"+jumpUrl;
        if(!confirm(msg)){
            return;
        }
        ...
        // 3.表单提交
        var infoId = $("#pop-ad_EditForm").find("input[name=infoId]").val();
        var menuId = $("#pop-ad_EditForm").find("input[name=menuId]").val();
        var title = $("#pop-ad_title").val();
        var subTitle = $("#pop-ad_subTitle").val();
        ...
        var code = $("#pop-ad_dataCoding").find('option:selected').val();
        var jumpType = $("#pop-ad_jumpType").find('option:selected').val();
        //var jumpUrl = $("#pop-ad_jumpUrl").val();
        ...
        var remarks = $("#pop-ad_remarks").val();
        ...
        $.ajax({
            ...
            success: function (data) {
                if(200 == data.code){
                    ...
                    // 显示之前的错误信息
                    $('#pop-ad_result').html(data.errorMessage);
                    $('#pop-ad_result').show();
                }
            }
        });
     ...
     * 校验表单
     */
    function validatePopAd() {
        // 1.校验名称和副标题
        var title = $("#pop-ad_title").val();
        if(isEmpty(title)){
            $('#pop-ad_result').html("名称不能为空！");
            $('#pop-ad_result').show();
            return false;
        }
        if(title.length > 100){
            $('#pop-ad_result').html("名称长度不能超过100！").show();
            return false;
        }
        if($("#pop-ad_subTitle").val().length > 150){
            $('#pop-ad_result').html("副标题长度不能超过150！").show();
            return false;
        }
        ...
        if(imgsLength <1){
            $('#pop-ad_result').html("图片不能为空！");
            $('#pop-ad_result').show();
            return false;
        }

        // 3.数据编码
        var dataCode = $("#pop-ad_dataCoding").find('option:selected').val();
        if(dataCode.length > 100){
            $('#pop-ad_result').html("数据编码长度不能超过100！").show();
            return false;
        }
        ...
        var jumpType = $("#pop-ad_jumpType").find('option:selected').val();
        if(isEmpty(jumpType)){
            $('#pop-ad_result').html("跳转类型不能为空！");
            $('#pop-ad_result').show();
            return false;
        }
        if(jumpType.length > 50){
            $('#pop-ad_result').html("跳转类型长度不能超过50！").show();
            return false;
        }
        ...
        if(!isEmpty(displayType)) {
            if (displayType.length > 50) {
                $('#pop-ad_result').html("显示频率长度不能超过50！").show();
                return false;
            }
        }
        ...
        if(!isEmpty(displayTimes)) {
            if (!(/^\d+$/).test(displayTimes)) {
                $('#pop-ad_result').html("显示次数必须为非负整数！");
                $('#pop-ad_result').show();
                return false;
            }
        }else{
        ...
        if(displayTimes > 999){
            $('#pop-ad_result').html("显示次数不能大于999！");
            $('#pop-ad_result').show();
            return false;
        }
        ...
        if(!isEmpty(dateParam)) {
            ...
            if(isEmpty(dateParamObj.beginTime)) {
                $('#pop-ad_result').html("时间设置开始日期不能为空！");
                $('#pop-ad_result').show();
                return false;
            }
        }
        ...
        if(remarks.length > 200){
            $('#pop-ad_result').html("备注长度不能超过200！").show();
            return false;
        }
...
function initTargetCrowd (targetCrowdId) {
    ...
    if(!flag && (targetCrowdId !=0 || targetCrowdId !="")){
        $('#pop-ad_result').html("请输入正确的目标人群id").show();
        return;
    }else{
        $('#pop-ad_result').html("").hide();
    }
